...
export const getById = query({
    ...
    handler: async (ctx, args) => {
        const identity = await ctx.auth.getUserIdentity();
        if (!identity) { // 这里指的是如果没有用户身份，抛出错误
            throw new Error("Unauthorized");
        }
        const userId = identity.subject;
        ...
        if (!document) { // 这里指的是如果文档不存在，抛出错误
            throw new Error("Document not found");
        }
        if (document.isPublished && !document.isArchived) { // 这里指的是如果文档已发布，但是没有归档，那么就返回文档
            return document;
        }
        if (document.userId !== userId) { // 这里指的是如果文档的用户id不等于用户id,抛出错误
            throw new Error("Unauthorized");
        }
...
export const update = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const {id, ...rest} = args; // 这里的...rest指的是剩余参数，也就是除了id之外的参数
